...
The skill teaches the canonical command for each step (e.g. `gh pr merge --auto --squash` rather than handcrafted API calls), the right credential helper for HTTPS git, and common pitfalls — fine-grained PAT permission errors, `--force` vs `--force-with-lease`, default-branch detection, etc.
...
Conforms to the [Agent Skills specification](https://agentskills.io/specification): SKILL.md at skill root, `scripts/` colocated, paths in SKILL.md relative to skill root.
...
### Hermes Agent (recommended — fetches scripts)

```sh
hermes skills install nishitpatel92/ai-agent-marketplace/skills/github
```

### Hermes Agent (URL — single-file, scripts not fetched)
...
Clone the skill directory anywhere on disk and point your framework at it:

```sh
git clone --depth=1 https://github.com/nishitpatel92/ai-agent-marketplace.git /tmp/m
cp -r /tmp/m/skills/github ~/your-framework-skills-dir/
```
...
```sh
python scripts/pr_status.py --repo owner/repo --pr 123              # one-shot snapshot
python scripts/pr_status.py --repo owner/repo --pr 123 --watch      # poll until terminal
python scripts/pr_status.py --repo owner/repo --pr 123 --json       # machine output
```
...
```sh
python scripts/pr_logs.py --repo owner/repo --pr 123 --out ./logs
```
